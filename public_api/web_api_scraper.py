...
def get_min_trade_qty(symbol):
    try:
        ...
        min_trade_qty = None
        if 'info' in market and 'lot_size_filter' in market['info']:
            min_trade_qty = float(market['info']['lot_size_filter']['min_trading_qty'])
        elif 'limits' in market and 'amount' in market['limits'] and 'min' in market['limits']['amount']:
            min_trade_qty = float(market['limits']['amount']['min'])
        return min_trade_qty
    except Exception as e:
        print(f"Error fetching min trade quantity for {symbol}: {e}")
        return None
...
for symbol in symbols_list:
    ...
    # Fetch 1-hour candle data for the past day (24 candles)
    candles_1h_past_day = exchange.fetch_ohlcv(symbol, timeframe='1h', limit=24)

    # Extract the volume data from the 1-hour candles
    volume_1h_past_day = [candle[5] for candle in candles_1h_past_day]

    # Append the volume data to the historical_volume_1h_past_day list
    historical_volume_1h_past_day.append(volume_1h_past_day)

    min_trade_qty = get_min_trade_qty(symbol)
    min_trade_quantities.append(min_trade_qty)
    
    

analysis_data = {'Assets': ['TRUUSDT', 'ETHWUSDT', 'IDUSDT', 'JOEUSDT', 'MASKUSDT', 'ALGOUSDT', 'WAVESUSDT', 'ARBUSDT', 'ETHUSD', 'ETH/USD:ETH', 'XEMUSDT', 'NKNUSDT', 'CFXUSDT', 'COCOSUSDT', 'HIGHUSDT', 'BLURUSDT', 'BUSDUSDT', 'HOOKUSDT', 'GFTUSDT', 'FETUSDT', 'COREUSDT', 'AGIXUSDT', 'ZECUSDT', 'IOTXUSDT', 'OMGUSDT', 'KSMUSDT', 'TRXUSDT', 'VETUSDT', 'ICPUSDT', 'CROUSDT', 'BTCUSD','BTC/USD:BTC','SHIB1000USDT','OCEANUSDT','GRTUSDT','CHZUSDT','SCUSDT','BLZUSDT','IMXUSDT','RSRUSDT','RNDRUSDT','LDOUSDT','ACHUSDT','ICXUSDT','GMTUSDT','GALAUSDT','1000BONKUSDT','BTCUSDT', 'FXSUSDT', 'ETHUSDT', 'BNBUSDT', 'XRPUSDT', 'BCHUSDT', 'MATICUSDT', 'DOTUSDT', 'ADAUSDT', 'LINKUSDT', 'FTMUSDT', 'DOGEUSDT', 'ATOMUSDT', 'AVAXUSDT', 'EOSUSDT', 'LTCUSDT', 'NEARUSDT', 'AXSUSDT', 'SANDUSDT', 'SOLUSDT', 'OPUSDT', 'APTUSDT', 'APEUSDT', 'ETCUSDT', 'GALUSDT', 'MANAUSDT', 'DYDXUSDT', 'SUSHIUSDT', 'XTZUSDT', 'HBARUSDT', 'LUNA2USDT', 'BITUSDT'],
'Price': prices_list,
'Min Trade Qty': min_trade_quantities,
...
df_what_negative_funding = df[df["Funding"] < 0][["Assets", "1m 1x Volume (USDT)", "Funding"]]
...
df_what_positive_funding = df[df["Funding"] > 0][["Assets", "1m 1x Volume (USDT)", "Funding"]]
...
# Calculate the total historical volume for each hour in the past day
total_historical_volume = [sum(hourly_volume) for hourly_volume in zip(*historical_volume_1h_past_day)]
