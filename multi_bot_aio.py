    ...
    while True:
        try:
            ...
            with thread_management_lock:
                ...
                # Handle target coins mode
                if target_coins_mode and whitelist:
                    for whitelisted_symbol in whitelist:
                        # Check if there's already a thread running for this symbol
                        long_thread_running = whitelisted_symbol in long_threads and long_threads[whitelisted_symbol][0].is_alive()
                        short_thread_running = whitelisted_symbol in short_threads and short_threads[whitelisted_symbol][0].is_alive()

                        if long_thread_running or short_thread_running:
                            logging.warning(f"Skipping {whitelisted_symbol} in target coins mode as there is already a running thread.")
                            continue  # Skip to the next symbol if there's already a running thread
