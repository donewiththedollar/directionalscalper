...
#         with general_rate_limiter:
#             market_maker.run_strategy(symbol, args.strategy, config, account_name, symbols_to_trade=symbols_allowed, rotator_symbols_standardized=latest_rotator_symbols, mfirsi_signal=mfirsi_signal, action=action)
...
#         long_completed = threading.Event()
#         short_completed = threading.Event()

#         with general_rate_limiter:
#             long_future = market_maker.run_strategy(symbol, args.strategy, config, account_name, symbols_to_trade=symbols_allowed, rotator_symbols_standardized=latest_rotator_symbols, mfirsi_signal=mfirsi_signal, completed_event=long_completed, side="long")
#             short_future = market_maker.run_strategy(symbol, args.strategy, config, account_name, symbols_to_trade=symbols_allowed, rotator_symbols_standardized=latest_rotator_symbols, mfirsi_signal=mfirsi_signal, completed_event=short_completed, side="short")
            
#             long_future.result()  # Wait for the long strategy to complete
#             short_future.result()  # Wait for the short strategy to complete

#         logging.info(f"Both long and short strategies for symbol {symbol} have completed.")
...
#                         with general_rate_limiter:
#                             mfirsi_signal = market_maker.get_mfirsi_signal(symbol)
#                         open_position_futures.append(trading_executor.submit(start_thread_for_symbol, symbol, args, manager, mfirsi_signal, "long"))
...
#                         with general_rate_limiter:
#                             mfirsi_signal = market_maker.get_mfirsi_signal(symbol)
#                         open_position_futures.append(trading_executor.submit(start_thread_for_symbol, symbol, args, manager, mfirsi_signal, "short"))
...
#                         with general_rate_limiter:
#                             mfirsi_signal = market_maker.get_mfirsi_signal(symbol)
#                         if not has_open_long and not long_thread_running:
#                             open_position_futures.append(trading_executor.submit(start_thread_for_open_symbol, symbol, args, manager, mfirsi_signal, has_open_long, has_open_short, long_mode, short_mode))
#                             logging.info(f"Submitted thread for symbol {symbol}. MFIRSI signal: {mfirsi_signal}. Has open long: {has_open_long}. Has open short: {has_open_short}.")
#                         if not has_open_short and not short_thread_running:
#                             open_position_futures.append(trading_executor.submit(start_thread_for_open_symbol, symbol, args, manager, mfirsi_signal, has_open_long, has_open_short, long_mode, short_mode))
#                             logging.info(f"Submitted thread for symbol {symbol}. MFIRSI signal: {mfirsi_signal}. Has open long: {has_open_long}. Has open short: {has_open_short}.")
#                     else:
