...
# price_precision, qty_precision = self.exchange.get_symbol_precision_bybit(symbol)
    ...
    def linear_grid_hardened_gridspan_orderbook_levels(
        ...
        try:
            ...
            # Calculate dynamic outer price distance based on order book (optional if you still need this elsewhere)
            ...
            # Use the new function to calculate grid levels based on the order book
            grid_levels = self.calculate_orderbook_based_grid_levels(
                order_book, current_price, long_pos_price, short_pos_price, levels, 
                dynamic_outer_price_distance, min_buffer_percentage, max_buffer_percentage
            )
