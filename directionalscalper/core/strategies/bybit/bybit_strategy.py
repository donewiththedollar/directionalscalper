...
# price_precision, qty_precision = self.exchange.get_symbol_precision_bybit(symbol)
    ...
    def linear_grid_hardened_gridspan_orderbook_maxposqty_properdca(self, symbol: str, open_symbols: list, total_equity: float, long_pos_price: float,
        ...
        try:
            ...
            # Get price levels from order book within the dynamic outer price distance (grid span)
            ask_prices = [price for price, qty in order_book['asks'] if price <= current_price * (1 + dynamic_outer_price_distance)]
            bid_prices = [price for price, qty in order_book['bids'] if price >= current_price * (1 - dynamic_outer_price_distance)]

            # Select grid levels based on the number of levels needed
            grid_levels_long = ask_prices[:levels] if len(ask_prices) >= levels else np.linspace(current_price - buffer_distance_long, current_price * (1 + dynamic_outer_price_distance), num=levels).tolist()
            grid_levels_short = bid_prices[:levels] if len(bid_prices) >= levels else np.linspace(current_price + buffer_distance_short, current_price * (1 - dynamic_outer_price_distance), num=levels).tolist()

            logging.info(f"[{symbol}] Long grid levels based on order book: {grid_levels_long}")
            logging.info(f"[{symbol}] Short grid levels based on order book: {grid_levels_short}")
    ...
    def linear_grid_handle_positions_mfirsi_persistent_notional_dynamic_buffer_qs(
        ...
        try:
            ...
            # Update TP for long position if long_pos_qty > 0:
            if long_pos_qty > 0:
                # Check for long positions
                if current_latest_time >= self.next_long_tp_update:
                    self.next_long_tp_update = self.update_quickscalp_tp(
            ...
            # Update TP for short position if short_pos_qty > 0:
            if short_pos_qty > 0:
                # Check for short positions
                if current_latest_time >= self.next_short_tp_update:
                    self.next_short_tp_update = self.update_quickscalp_tp(
        ...
        try:
            ...
            if long_pos_qty > 0:
                ...
                if price_change_pct_long > required_price_move_long_pct:
                    ...
                    logging.info(f"[{symbol}] Price change does not exceed dynamic outer price distance percentage for long position. No need to replace long grid.")
            ...
            if short_pos_qty > 0:
                ...
                if price_change_pct_short > required_price_move_short_pct:
                    ...
                    logging.info(f"[{symbol}] Price change does not exceed dynamic outer price distance percentage for short position. No need to replace short grid.")
    ...
    #     current_pos_value = (long_pos_qty if side == 'buy' else short_pos_qty) * (best_ask_price if side == 'buy' else best_bid_price)
    #     adjusted_max_position_value = max_position_value - current_pos_value
    ...
    #     current_position_qty = long_pos_qty if long_pos_qty > 0 else short_pos_qty
    #     total_amount_adjusted = total_amount + (current_position_qty * current_price)
