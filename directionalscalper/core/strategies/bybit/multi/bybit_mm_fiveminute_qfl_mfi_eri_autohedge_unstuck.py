...
class BybitMMFiveMinuteQFLMFIERIAutoHedgeUnstuck(Strategy):
    def __init__(self, exchange, manager, config, symbols_allowed=None):
        ...
        self.last_known_ask_lock = threading.Lock()
        self.last_known_bid_lock = threading.Lock()
    ...
    def update_last_known_prices(self, symbol, best_ask_price, best_bid_price):
        # Safely update last known ask price
        with self.last_known_ask_lock:
            self.last_known_ask[symbol] = best_ask_price

        # Safely update last known bid price
        with self.last_known_bid_lock:
            self.last_known_bid[symbol] = best_bid_price
    ...
    def run_single_symbol(self, symbol, rotator_symbols_standardized=None):
        ...
        try:
            ...
            logging.info(f"[Thread ID: {current_thread_id}] Acquired lock and running strategy for symbol: {symbol}")
            ...
            while True:
                ...
                # Safely update last known prices
                self.update_last_known_prices(symbol, best_ask_price, best_bid_price)
                ...
                if symbol in open_symbols and trading_allowed:
                    ...
                    self.bybit_entry_mm_5m_with_qfl_mfi_and_auto_hedge_with_eri(
                        open_orders, 
                        symbol, 
                        trend, 
                        hma_trend, 
                        ...
                        long_pos_price, 
                        short_pos_price, should_long, should_short, should_add_to_long, should_add_to_short, hedge_ratio, price_difference_threshold)
