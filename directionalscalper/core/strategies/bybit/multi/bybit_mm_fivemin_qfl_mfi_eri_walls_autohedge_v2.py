...
logging = Logger(logger_name="BybitMMFiveMinuteQFLMFIERIAutoHedgeWallsV2", filename="BybitMMFiveMinuteQFLMFIERIAutoHedgeWallsV2.log", stream=True)

symbol_locks = {}

class BybitMMFiveMinuteQFLMFIERIAutoHedgeWallsV2(Strategy):
        ...
        while True:
            ...
            if symbol in rotator_symbols_standardized or (symbol in open_symbols or trading_allowed): # and instead of or
                ...
                if long_pos_qty > 0 and long_take_profit is not None and tp_order_counts['long_tp_count'] == 0:
                    ...
                    self.bybit_hedge_placetp_maker_v2(symbol, long_pos_qty, long_take_profit, positionIdx=1, order_side="sell", open_orders=open_orders)
                ...
                if short_pos_qty > 0 and short_take_profit is not None and tp_order_counts['short_tp_count'] == 0:
                    ...
                    self.bybit_hedge_placetp_maker_v2(symbol, short_pos_qty, short_take_profit, positionIdx=2, order_side="buy", open_orders=open_orders)
                ...
                if long_pos_qty > 0:
                    if current_time >= self.next_long_tp_update and long_take_profit is not None:
                        self.next_long_tp_update = self.update_take_profit_spread_bybit_v2(
                            symbol=symbol, 
                ...
                if short_pos_qty > 0:
                    if current_time >= self.next_short_tp_update and short_take_profit is not None:
                        self.next_short_tp_update = self.update_take_profit_spread_bybit_v2(
                            symbol=symbol, 
