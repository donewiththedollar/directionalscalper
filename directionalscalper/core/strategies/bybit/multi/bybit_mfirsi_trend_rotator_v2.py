...
logging = Logger(logger_name="BybitMFIRSITrendRotator2", filename="BybitMFIRSITrendRotator2.log", stream=True)

class BybitMFIRSITrendRotatorv2(Strategy):
    ...
    #     running_symbols = set()  # Use a set to keep track of the symbols currently being traded

    #     while True:
    #         rotator_symbols = set(self.manager.get_auto_rotate_symbols())

    #         # Starting trading operations for new symbols
    #         new_symbols = rotator_symbols - running_symbols
    #         for symbol in new_symbols:
    #             self.executor.submit(self.run_single_symbol, symbol)

    #         # Update running symbols
    #         running_symbols = rotator_symbols
    ...
    def run_single_symbol(self, symbol):
        ...
        while True:
            ...
            if symbol not in rotator_symbols:
                logging.info(f"Symbol {symbol} no longer in rotator symbols. Managing open positions.")
                self.manage_non_rotating_symbol(symbol)
            ...
            if symbol not in whitelist or symbol in blacklist:
                logging.info(f"Symbol {symbol} is no longer allowed based on whitelist/blacklist. Managing open positions.")
                self.manage_non_rotating_symbol(symbol)
                continue  # Skips to the next iteration of the loop
